...
# GBM
gbm_cg <- read_tsv("../results/methylation/sver54.gene.CG.gff", col_names = NULL) |>
  filter(X3 == "exon") |>
  mutate(
    gene = str_extract(X9, "Parent=([^;]+)", group = 1),
    length = X5 - X4,
...
gbm_chg <- read_tsv("../results/methylation/sver54.gene.CHG.gff", col_names = NULL) |>
  filter(X3 == "exon") |>
  mutate(
    type = "CHG",
    gene = str_extract(X9, "Parent=([^;]+);", group = 1),
...
gbm_merged <- inner_join(gbm_cg, gbm_chg, by = "gene") |>
  mutate(
...
ggsave("plots/genome_features.svg", width = 200, height = 100, units = "mm")
```

## Gene body methylation

```{r}
plot <- ggplot(gbm_merged, aes(x = cg, y = chg, colour = classification)) +
  geom_scattermore(alpha = .05, pointsize = 3.2) +
  scale_colour_tableau(guide = guide_legend(override.aes = list(alpha = 1)))

ggsave("plots/gbm.svg", plot, width = 75, height = 100, units = "mm")
```
